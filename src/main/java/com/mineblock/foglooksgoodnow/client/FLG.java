...
public class FLG implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        FogEvents.RENDER_FOG.register((fogMode, fogType, camera, v, v1, v2, v3, fogShape, fogData) -> {
            if (camera.getFluidInCamera() == FogType.NONE) {
                ...
                if (FogManager.shouldRenderCaveFog()) {
                    ...
                    fogStart = densityManager.caveFogStart.get(v);
                } else fogStart = densityManager.fogStart.get(v);

                RenderSystem.setShaderFogStart(v1 * fogStart);
        ...
        CONFIG.subscribeToCaveFogStart(col -> FogManager.getDensityManagerOptional().ifPresent((fogDensityManager -> fogDensityManager.initializeConfig())));
